...
class Main_Menu:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.help_bt.checkForInput(pygame.mouse.get_pos()):
                        self.help()
                    if self.level_editor_bt.checkForInput(pygame.mouse.get_pos()):
                        self.level_select(level_editor=True)
    ...
    def level_select(self, level_editor=False):
        # design
        level_bt_1design = pygame.image.load("data/Design_menu/level_button1.png")  # design кнопки для уровня
        level_bt_1design = pygame.transform.scale(level_bt_1design, (300, 300))
        level_bt_2design = pygame.image.load("data/Design_menu/level_button2.png")  # design кнопки для уровня
        level_bt_2design = pygame.transform.scale(level_bt_2design, (300, 300))
        level_bt_3design = pygame.image.load("data/Design_menu/level_button3.png")  # design кнопки для уровня
        level_bt_3design = pygame.transform.scale(level_bt_3design, (300, 300))
        level_bt_4design = pygame.image.load("data/Design_menu/exit.png")  # design кнопки для уровня
        level_bt_4design = pygame.transform.scale(level_bt_4design, (300, 300))
        level1_bt = Button(level_bt_1design, int(self.screen.get_width() * 0.3), 250, "level 1", main_font)
        level2_bt = Button(level_bt_2design, int(self.screen.get_width() * 0.5), 250, "level 2", main_font)
        level3_bt = Button(level_bt_3design, int(self.screen.get_width() * 0.8), 250, "level 3", main_font)
        exit = Button(level_bt_4design, self.screen.get_width() // 2, int(self.screen.get_height() * 0.9), "exit",
                      main_font)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if not level_editor:
                        if level1_bt.checkForInput(pygame.mouse.get_pos()):
                            Game(level_num=1).run()
                            self.level_start()
                        elif level2_bt.checkForInput(pygame.mouse.get_pos()):
                            Game(level_num=2).run()
                            self.level_start()
                        elif level3_bt.checkForInput(pygame.mouse.get_pos()):
                            Game(level_num=3).run()
                            self.level_start()
                    else:
                        if level1_bt.checkForInput(pygame.mouse.get_pos()):
                            self.editor_run(level_num=1)
                        elif level2_bt.checkForInput(pygame.mouse.get_pos()):
                            self.editor_run(level_num=2)
                        elif level3_bt.checkForInput(pygame.mouse.get_pos()):
                            self.editor_run(level_num=3)
    ...
    def editor_run(self, level_num):
        Editor(level_num=level_num).run()
